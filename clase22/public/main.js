...
testProducts.addEventListener('click', handleSubmit_testProducts)
...
function handleSubmit_testProducts(event){
  /* event.preventDefault() */
  window.location = "http://localhost:8080/test/products"
}
...
function renderChatMessagesNormalized(data) {
  ...
  const reductionPercentage = Math.floor(
    100 -
    ((JSON.stringify(dataDenormalized).length) * 100 /
